...
def test_rewrite_replaces_the_document_and_keeps_its_siblings(tmp_path):
    # Re-normalisation rebuilds the two files it owns from the docling.json
    # already on disk; the conversion's own output and the figures beside
    # it are untouched, because Docling did not run again.
    store = Store(tmp_path)
    path = store.write("a" * 64, "r1", a_document(), "# Old", {"schema_name": "x"})
    (path / "images").mkdir()
    (path / "images" / "image_1_abc.png").write_bytes(b"figure")

    store.rewrite("a" * 64, "r1", a_document(), "# New")

    artifact = store.read("a" * 64, "r1")
    assert artifact.markdown == "# New"
    assert json.loads((artifact.path / "docling.json").read_text())["schema_name"] == "x"
    assert (artifact.path / "images" / "image_1_abc.png").read_bytes() == b"figure"


def test_rewrite_refuses_an_object_that_is_not_there(tmp_path):
    store = Store(tmp_path)
    with pytest.raises(PageboundError):
        store.rewrite("a" * 64, "r1", a_document(), "# New")
