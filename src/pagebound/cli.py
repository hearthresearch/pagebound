...
from pagebound.sync import ZoteroItem, iter_items, sync_items, zotero_client
...
def sync(
    ...
    with progress:
        ...
        counts = sync_items(
            iter_items(client, storage_root=storage, library=key, collection=collection),
            store=store, index=index, recipe=recipe, force=force,
