...
def sync(
    ...
    if pairs is not None:
        ...
        missing = [pair for pair in pairs if pair[1] not in found]
    ...
    # Only now, with something already wrong, is it worth a second
    # request: the key at hand is often the attachment's, since that is
    # what names the directory under Zotero's storage.
    attachments = (attachment_parents(client, [key for _, key in missing])
                   if missing else {})
    for ref, item_key in missing:
        parent = attachments.get(item_key)
        if parent:
            errors.print(f"[red]{ref} is an attachment of {parent}; sync that[/]")
        else:
...
def renormalize(
    ...
    """Rebuild cached documents from the Docling output already stored.

    When a pagebound release changes how Docling's JSON is read, the
    artifacts already in the cache still describe the old reading. This
    re-reads each stored `docling.json` and rewrites the document and
    markdown beside it, which takes milliseconds a paper and never runs
    Docling. The conversion's own timestamp and its figures are kept.

    Only artifacts whose output actually changes are written, so running
    it twice is not running it twice.

    Prints "rebuilt N of M artifacts"; with --dry-run, "would rebuild".
    """
    report = api.renormalize(store=Store(cache), dry_run=dry_run)
    verb = "would rebuild" if dry_run else "rebuilt"
    console.print(f"[bold]{verb} {report.rewritten} of {report.scanned} "
                  f"artifacts[/]; {report.unchanged} already current")
    if report.failed:
        console.print(f"[yellow]{report.failed} could not be read and were left "
                      f"alone[/]")
