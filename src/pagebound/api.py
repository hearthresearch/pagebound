...
class RenormalizeReport:
    ...
    `scanned` counts artifacts, not directories: a tombstone has no
    Docling output to re-read and is not one.
    """
...
def renormalize(
    ...
    *,
    ...
    """Rebuild every cached document from the Docling output beside it.

    A change to how this layer reads Docling's JSON leaves the artifacts
    already in the store describing the old reading. The recipe key
    deliberately excludes `format_version`, so nothing invalidates them
    and a reconversion would pay thirty seconds a paper to get output
    Docling already produced. This re-reads the stored `docling.json`
    instead, which costs milliseconds and never asks Docling anything.

    An artifact is rewritten only when the result actually differs, so
    the pass is idempotent and safe to repeat. `converted_at` is kept,
    because Docling did not run again; `pagebound_version` moves to the
    version that produced the blocks, which is what it names.
    """
    store = store or Store()
    scanned = rewritten = unchanged = failed = 0
    for sha, recipe_key in store.iter_objects():
        directory = store.object_dir(sha, recipe_key)
        docling_file = directory / "docling.json"
        if not store.has(sha, recipe_key) or not docling_file.exists():
            continue
        scanned += 1
        try:
            artifact = store.read(sha, recipe_key)
            docling = json.loads(docling_file.read_text(encoding="utf-8"))
            document, markdown = normalize(
                docling,
                source_sha256=artifact.document.source.sha256,
                conversion=artifact.document.conversion,
                probe=_probe_of(artifact.document.source),
            )
        except (OSError, ValueError, KeyError, PageboundError):
            failed += 1
            continue
        if markdown == artifact.markdown and document.blocks == artifact.document.blocks:
            unchanged += 1
            continue
        rewritten += 1
        if dry_run:
            continue
        document = replace(document, conversion=replace(
            document.conversion, pagebound_version=PAGEBOUND_VERSION))
        store.rewrite(sha, recipe_key, document, markdown)
    return RenormalizeReport(scanned=scanned, rewritten=rewritten,
                             unchanged=unchanged, failed=failed)


def _probe_of(source: Any) -> dict[str, Any] | None:
    """The recorded text-layer verdict, in the shape `normalize` takes.

    Carried across rather than re-probed: the PDF may not be on this
    machine any more, and the verdict belongs to the conversion that
    ran.
    """
    if source.text_layer is None:
        return None
    return {"has_text": source.text_layer,
            "probe_pages": source.probe_pages,
            "probe_chars": source.probe_chars}
