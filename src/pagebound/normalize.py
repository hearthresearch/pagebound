...
def _captions(
    docling: dict[str, Any], item: dict[str, Any], seen: set[str]
) -> Iterator[dict[str, Any]]:
    """The captions of a table or picture, which the body tree cannot reach.

    Docling parents a caption to the item it describes, not to the body:
    the text's `parent` is `#/tables/0` and the table names it in
    `captions`, so a walk of `body.children` never arrives at it. Reading
    `captions` rather than `children` is deliberate: a picture's
    `children` also holds the glyph runs Docling assigned to the figure
    region, which on one measured paper were 345 fragments of the
    journal masthead ("C", "ommunications of the").
    """
    for caption in item.get("captions") or []:
        ref = caption.get("$ref") if isinstance(caption, dict) else None
        if not ref or ref in seen:
            continue
        resolved = _resolve(docling, ref)
        if resolved is None:
            continue
        seen.add(ref)
        yield resolved
...
def _walk(
    ...
    A captioned item yields its caption first, which is the order
    docling-core's own markdown serializer uses and where 21 of 23
    measured table captions sat on the page. `seen` keeps an item to one
    block when a Docling release lists it both in `body.children` and in
    an item's `captions`; a duplicate would shift every offset after it.
    """
    ...
    for child in node.get("children", []) or []:
        ...
        if ref.startswith("#/groups/"):
            yield from _walk(docling, item, seen)
        else:
            yield from _captions(docling, item, seen)
            yield item
